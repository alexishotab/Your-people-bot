...
class Database:
    ...
    def add_user(self,chat_id,Gender, Age,Name):
        self.cursor.execute("INSERT INTO `users` (`chat_id`,`gender`,`age`,`name`) VALUES(?,?,?,?)",(chat_id,Gender,Age,Name))
        self.connection.commit()

    def create_self_anketa(self,chat_id):
        with self.connection:
            anket=self.cursor.execute("SELECT `name`,`age`,`gender` from `users` WHERE `chat_id`=?",(chat_id,))
            if anket!=[]:
                for row in anket:
